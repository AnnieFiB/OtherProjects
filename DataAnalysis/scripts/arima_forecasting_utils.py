...
def suggest_arima_params(series_dict, lags=20, alpha=0.05):
    suggestions = {}

    def get_first_significant_lag(values, conf_int):
        for i in range(1, len(values)):  # Skip lag 0
            lower = conf_int[i][0]
            upper = conf_int[i][1]
            if values[i] <= lower or values[i] >= upper:
                return i
        return 1  # Default to 1 if no clear spike is found

    for ticker, series in series_dict.items():
        cleaned = series.dropna()

        pacf_vals, pacf_conf = pacf(cleaned, nlags=lags, alpha=alpha)
        acf_vals, acf_conf = acf(cleaned, nlags=lags, alpha=alpha)

        p = get_first_significant_lag(pacf_vals, pacf_conf)
        q = get_first_significant_lag(acf_vals, acf_conf)

        suggestions[ticker] = {"p": p, "d": 0, "q": q}
        print(f"{ticker} → Suggested ARIMA(p,d,q): ({p}, 0, {q})")
