...
def plot_forecast(forecast, city_name, air_forecast=None):
    """Display single city forecast with all plots in one figure"""
    if not forecast or not forecast.get("list"):
        return None
    ...
    try:
        ...
        if air_forecast and air_forecast.get("list"):
            ...
            aqi_df = aqi_df.groupby("datetime").mean().reset_index()

        fig, axs = plt.subplots(4 if aqi_df is not None else 3, 1, figsize=(12, 10), sharex=True)

        axs[0].plot(df["datetime"], df["temp"], 'r-')
        axs[0].set_ylabel('Temp (°C)')
        axs[0].grid(True)
        axs[0].set_title(f"{city_name} - Forecast Overview")

        axs[1].plot(df["datetime"], df["humidity"], 'b-')
        axs[1].set_ylabel('Humidity (%)')
        axs[1].grid(True)

        axs[2].bar(df["datetime"], df["rain"], width=0.05, color='blue', alpha=0.5)
        axs[2].set_ylabel('Rain (mm)')
        axs[2].grid(True)

        if aqi_df is not None:
            axs[3].plot(aqi_df["datetime"], aqi_df["aqi"], 'g-o')
            axs[3].set_ylabel('AQI')
            axs[3].grid(True)
